"""CREATE OR REPLACE PROCEDURE insert_or_update_user(IN p_name VARCHAR, IN p_phone VARCHAR)
LANGUAGE plpgsql
AS $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM phonebook WHERE username = p_name) THEN
        ...
        RAISE NOTICE 'Телефон для пользователя "%" обновлен на "%".', p_name, p_phone;
    ELSE
        -- Если не существует, вставляем нового пользователя
        INSERT INTO phonebook (username, phone)
        VALUES (p_name, p_phone);
        RAISE NOTICE 'Новый пользователь "%" добавлен с телефоном "%".', p_name, p_phone;
    END IF;
END;
$$;
"""
...
def main_menu():
    while True:
        ...
        print("8. Вставить или обновить пользователя  (SQL-функция)")
